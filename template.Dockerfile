FROM oven/bun:{CURRENT_BUN_VERSION} as bun
...
RUN ARCH=$(arch); if [ $ARCH != 'x86_64' ]; then echo "[NODE]Invalid platform, use '--platform linux/amd64'"; exit 1; fi;
...
FROM bun as amd64
RUN ARCH=$(arch); if [ $ARCH != 'x86_64' ]; then echo "[BUN]Invalid platform, use '--platform linux/amd64'"; exit 1; fi;
...
RUN ARCH=$(arch); if [ $ARCH != 'aarch64' ]; then echo "[NODE]Invalid platform, use '--platform linux/arm64'"; exit 1; fi;
WORKDIR /node-binaries
ARG NODEJS_URL
RUN if [ "$NODEJS_URL" == '' ]; then echo "use '--build-arg NODEJS_URL=<nodejs-url>'"; exit 1; fi;
RUN wget $NODEJS_URL
...
FROM bun as arm64
RUN ARCH=$(arch); if [ $ARCH != 'aarch64' ]; then echo "[BUN]Invalid platform, use '--platform linux/arm64'"; exit 1; fi;
